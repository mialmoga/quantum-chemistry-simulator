    ...
    createPanel() {
        ...
        advancedBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                advancedBtn.style.background = 'rgba(100, 255, 100, 0.1)';
                advancedBtn.style.borderColor = 'rgba(100, 255, 100, 0.3)';
                advancedBtn.style.color = '#90ee90';
                
                // Show hint
                if(window.showHint) {
                    window.showHint('🔬 Datos avanzados cargados (40+ propiedades por elemento)');
                }
            } catch(error) {
                advancedBtn.innerHTML = '❌ Error al cargar';
                advancedBtn.style.background = 'rgba(255, 100, 100, 0.1)';
                console.error('Failed to load advanced data:', error);
            }
        });
        
        advancedBtn.addEventListener('mouseenter', () => {
            if(!advancedBtn.disabled) {
                advancedBtn.style.background = 'rgba(100, 200, 255, 0.2)';
            }
        });
        advancedBtn.addEventListener('mouseleave', () => {
            if(!advancedBtn.disabled) {
                advancedBtn.style.background = 'rgba(100, 200, 255, 0.1)';
            }
        });
